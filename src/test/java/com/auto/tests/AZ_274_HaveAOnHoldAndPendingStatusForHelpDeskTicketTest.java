...
public class AZ_274_HaveAOnHoldAndPendingStatusForHelpDeskTicketTest extends TestCore {
	...
	@Test
	public void az__274_HaveAOnHoldAndPendingStatusForHelpDeskTicket(){
		...
		assetPage.clickOnViewEditDelIcons(InventoryPageObject.searchTableId,HelpDeskTicketsPageObject.ticketName, 2, "Edit", 10, "Ticket", 2);
		
		assetPage.clickOnArrowIconOfFields("Status", 8);
		assertTrue(ticketPage.isSuggestionListAppearOnStatus(HelpDeskTicketsPageObject.listStatus_xpath, "On Hold"));
		assertTrue(ticketPage.isSuggestionListAppearOnStatus(HelpDeskTicketsPageObject.listStatus_xpath, "Pending"));
		assetPage.enterSearchString("Pending", 1, "Status");
		signup.clickWithScroll("Save");
		
		dashboard.clickButton("All Tickets");
		
		ticketPage.clickOnStausColumn();
		assertTrue(ticketPage.isSuggestionListAppearOnStatus(HelpDeskTicketsPageObject.status_xpath, "On Hold"));
		assertTrue(ticketPage.isSuggestionListAppearOnStatus(HelpDeskTicketsPageObject.status_xpath, "Pending"));
		ticketPage.clickOnStatusFilterIcon("Pending");
		ticketPage.clickOnApplyButton();
		
		assertTrue(assetPage.isItemAddedInGrid(InventoryPageObject.searchTableId, "Ticket",HelpDeskTicketsPageObject.ticketName,2),"Expected ticket does not present on grid");
		
		assetPage.clickOnViewEditDelIcons(InventoryPageObject.searchTableId,HelpDeskTicketsPageObject.ticketName, 2, "Edit", 10, "Ticket", 2);
		assetPage.clickOnArrowIconOfFields("Status", 8);
		assetPage.enterSearchString("On Hold", 1, "Status");
		signup.clickWithScroll("Save");
		
		dashboard.clickButton("All Tickets");
		
		ticketPage.clickOnStausColumn();
		ticketPage.clickOnStatusFilterIcon("On Hold");
		ticketPage.clickOnApplyButton();
