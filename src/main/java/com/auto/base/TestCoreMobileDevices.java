...
import com.auto.configproperties.ConfigMobileDevicesProperties;
...
public class TestCoreMobileDevices extends Page {
	...
	public void fetchSuiteConfiguration(ITestContext testContext) throws IOException, InterruptedException, MalformedURLException {
		...
		targetBrowser = ConfigMobileDevicesProperties.browser_name;
		os = ConfigMobileDevicesProperties.OS;
		crossBrowser = ConfigMobileDevicesProperties.crossBrowser;
		
		
		browserVersion = ConfigMobileDevicesProperties.browser_version;
		os_Platform = ConfigMobileDevicesProperties.platform;
		screen_Resolution = ConfigMobileDevicesProperties.screenResulution;
		...
		if (crossBrowser.toLowerCase().equals("local")) {
			...
			if (os.toLowerCase().equals("windows")) {
		       ...
		       if (targetBrowser.toLowerCase().contains("chrome")) {
				 ...
				 caps.setCapability(MobileCapabilityType.DEVICE_NAME,ConfigMobileDevicesProperties.realDevice_name);
				 caps.setCapability(MobileCapabilityType.VERSION, ConfigMobileDevicesProperties.device_version);
			...
			if(os.toLowerCase().equals("windows")){
			 	...
			 	DesiredCapabilities caps = new DesiredCapabilities();
			 	caps.setCapability("browserName", ConfigMobileDevicesProperties.browser_name);
			 	caps.setCapability("deviceName", ConfigMobileDevicesProperties.device_name);
			 	caps.setCapability("platformVersion", ConfigMobileDevicesProperties.device_version);
			 	caps.setCapability("platformName", ConfigMobileDevicesProperties.OS);
